...
function UpdatePlayerTable(connectedPlayers)
	local num = {}, 1
	local ems, police, sheriff, taxi, mechanic, dadgostari, weazel, players = 0, 0, 0, 0, 0, 0, 0, 0
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, sheriff = sheriff, taxi = taxi, mechanic = mechanic, dadgostari = dadgostari, weazel = weazel, player_count = players}
	})
...
AddEventHandler('onKeyUP',function(key)
	ESX.TriggerServerCallback('esx_aduty:getAdminPerm', function(perm)
		if key == 'f10' and (perm == 0 or not ESX.GetPlayerData().aduty) then
			ToggleScoreBoard()
			TriggerEvent('InteractSound_CL:PlayOnOne', 'open', 0.1)
			Citizen.Wait(100)
		end
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		SendNUIMessage({
			action = 'updateServerInfo',
			playTime = string.format("%02d %02d", playHour, playMinute)
		})
	end
end)
